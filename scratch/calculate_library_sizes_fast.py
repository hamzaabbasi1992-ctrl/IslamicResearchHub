...
print(f"{'Maktaba (Library) Name':<35} | {'Books':<8} | {'Pages':<12} | {'Est. Text Size':<15} | {'Est. DB Storage':<16} | {'Share (%)':<8}")
print("-" * 108)
...
for r in rows:
    ...
    share_pct = (pages_cnt / total_pages_sum * 100) if total_pages_sum > 0 else 0
    est_db_mb = (db_file_size_mb * (share_pct / 100)) if total_pages_sum > 0 else 0
