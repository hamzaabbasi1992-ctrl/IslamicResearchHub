"""Tests for SlideDeckGenerationWorker's chunk-by-chunk generation, off the GUI thread."""
...
from islamic_research_hub.interfaces.desktop_app.slide_deck_worker import (  # noqa: E402
    SlideDeckGenerationWorker,
)

_EMPTY_JSON = "[]"
_ONE_SLIDE_JSON = '[{"title": "A Slide", "bullets": ["A real point."]}]'
...
class _FakeService:
    """A fake AiAgentService.generate_slide_deck() - mirrors
    ...
    def __init__(
        ...
        worker: SlideDeckGenerationWorker | None = None,
    ) -> None:
...
def test_multi_chunk_generation_collects_real_slides_in_order_and_emits_progress(qtbot) -> None:
    service = _FakeService([_ONE_SLIDE_JSON, _EMPTY_JSON, _ONE_SLIDE_JSON])
    worker = SlideDeckGenerationWorker(lambda: service, book_id=1, chunks=((1, 10), (11, 20), (21, 30)))
    progress_calls = []
    ...
    slides = blocker.args[0]
    assert len(slides) == 2  # 2 real slides collected (chunk 2 had none)
...
def test_no_service_available_emits_unavailable_and_collects_nothing(qtbot) -> None:
    worker = SlideDeckGenerationWorker(lambda: None, book_id=1, chunks=((1, 10),))
    unavailable_calls = []
...
def test_a_failing_chunk_is_skipped_not_fatal_to_the_rest(qtbot) -> None:
    service = _FakeService([_ONE_SLIDE_JSON, _ONE_SLIDE_JSON, _ONE_SLIDE_JSON], fail_at=1)
    worker = SlideDeckGenerationWorker(lambda: service, book_id=1, chunks=((1, 10), (11, 20), (21, 30)))
    ...
    slides = blocker.args[0]
    assert len(slides) == 2  # chunk 1 failed and was skipped
    assert len(service.calls) == 3


def test_cancellation_requested_mid_run_stops_further_chunks(qtbot) -> None:
    worker = SlideDeckGenerationWorker(lambda: None, book_id=1, chunks=((1, 10), (11, 20), (21, 30)))
    service = _FakeService(
        [_ONE_SLIDE_JSON, _ONE_SLIDE_JSON, _ONE_SLIDE_JSON],
        cancel_worker_at=0,
        worker=worker,
    )
