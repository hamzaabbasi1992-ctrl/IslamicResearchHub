...
def test_generate_mcqs_seeds_the_right_book_and_page_range() -> None:
    provider = FakeLLMProvider([LLMTurn(text="[]", tool_calls=(), stop_reason="end_turn")])
    service = AiAgentService(provider, _executor())

    result = service.generate_mcqs(book_id=42, start_page=10, end_page=20)
...
def test_generate_mcqs_uses_its_own_system_prompt_not_shared_with_others() -> None:
    provider = FakeLLMProvider([LLMTurn(text="[]", tool_calls=(), stop_reason="end_turn")])
    service = AiAgentService(provider, _executor())

    service.generate_mcqs(book_id=1, start_page=1, end_page=5)

    system_prompt, _messages = provider.calls[0]
    assert "JSON" in system_prompt
    assert "multiple-choice" in system_prompt.lower() or "correct_index" in system_prompt.lower()


def test_generate_mcqs_rejects_a_backwards_page_range() -> None:
    service = AiAgentService(FakeLLMProvider([]), _executor())

    with pytest.raises(ValueError):
        service.generate_mcqs(book_id=1, start_page=20, end_page=10)
...
def test_generate_lecture_notes_seeds_the_right_book_and_page_range() -> None:
    provider = FakeLLMProvider([LLMTurn(text="[]", tool_calls=(), stop_reason="end_turn")])
    service = AiAgentService(provider, _executor())

    result = service.generate_lecture_notes(book_id=42, start_page=10, end_page=20)
...
def test_generate_lecture_notes_uses_its_own_system_prompt_not_shared_with_others() -> None:
    provider = FakeLLMProvider([LLMTurn(text="[]", tool_calls=(), stop_reason="end_turn")])
    service = AiAgentService(provider, _executor())

    service.generate_lecture_notes(book_id=1, start_page=1, end_page=5)

    system_prompt, _messages = provider.calls[0]
    assert "JSON" in system_prompt
    assert "lecture" in system_prompt.lower()


def test_generate_lecture_notes_rejects_a_backwards_page_range() -> None:
    service = AiAgentService(FakeLLMProvider([]), _executor())

    with pytest.raises(ValueError):
        service.generate_lecture_notes(book_id=1, start_page=20, end_page=10)
