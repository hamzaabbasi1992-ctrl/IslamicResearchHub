"""Tests for FlashcardExtractionWorker's chunk-by-chunk generation, off the GUI thread."""
...
from islamic_research_hub.interfaces.desktop_app.flashcard_extraction_worker import (  # noqa: E402
    FlashcardExtractionWorker,
)

_EMPTY_JSON = "[]"
_ONE_FLASHCARD_JSON = (
    '[{"front": "A question", "back": "An answer", '
...
class _FakeService:
    """A fake AiAgentService.generate_flashcards() - mirrors
    test_event_extraction_worker.py's _FakeService exactly."""
    ...
    def __init__(
        ...
        worker: FlashcardExtractionWorker | None = None,
    ) -> None:
...
def test_multi_chunk_generation_stores_real_flashcards_and_emits_progress(
    tmp_path: Path, qtbot
) -> None:
    database_path = tmp_path / "books.db"
    _seed_book(database_path)
    repository = FlashcardCandidateRepository(database_path)
    service = _FakeService([_ONE_FLASHCARD_JSON, _EMPTY_JSON, _ONE_FLASHCARD_JSON])
    ...
    assert blocker.args == [2]  # 2 real flashcards stored (chunk 2 had none)
...
def test_no_service_available_emits_unavailable_and_stores_nothing(tmp_path: Path, qtbot) -> None:
    database_path = tmp_path / "books.db"
    repository = FlashcardCandidateRepository(database_path)
    worker = FlashcardExtractionWorker(lambda: None, repository, book_id=1, chunks=((1, 10),))
    unavailable_calls = []
...
def test_a_failing_chunk_is_skipped_not_fatal_to_the_rest(tmp_path: Path, qtbot) -> None:
    ...
    service = _FakeService(
        [_ONE_FLASHCARD_JSON, _ONE_FLASHCARD_JSON, _ONE_FLASHCARD_JSON], fail_at=1
    )
    worker = FlashcardExtractionWorker(
        lambda: service, repository, book_id=1, chunks=((1, 10), (11, 20), (21, 30))
    )
...
def test_cancellation_requested_mid_run_stops_further_chunks(tmp_path: Path, qtbot) -> None:
    ...
    service = _FakeService(
        [_ONE_FLASHCARD_JSON, _ONE_FLASHCARD_JSON, _ONE_FLASHCARD_JSON],
        cancel_worker_at=0,
        worker=worker,
    )
