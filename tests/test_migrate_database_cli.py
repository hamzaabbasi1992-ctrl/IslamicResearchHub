...
def test_main_applies_pending_migrations_and_reports_them(tmp_path: Path, capsys) -> None:
    ...
    assert "Applied 8:" in captured.out
    assert "Applied 9:" in captured.out
    assert "Applied 10:" in captured.out
    assert "Applied 11:" in captured.out
    assert "Applied 12:" in captured.out
    assert "Applied 13:" in captured.out
    assert "Applied 14:" in captured.out
    assert "Applied 15:" in captured.out
    assert "Applied 16:" in captured.out
    assert "Applied 17:" in captured.out
    assert "Applied 18:" in captured.out
    assert "Applied 19:" in captured.out
    assert "Version after: 19" in captured.out
