"""Tests for EventExtractionWorker's chunk-by-chunk extraction, off the GUI thread."""
...
from islamic_research_hub.interfaces.desktop_app.event_extraction_worker import (  # noqa: E402
    EventExtractionWorker,
)

_EMPTY_JSON = "[]"
_ONE_EVENT_JSON = (
    '[{"title": "An Event", "alternate_names": [], "subject": "topic", '
    '"date_hijri": null, "date_gregorian": null, "location": null, '
    '"background": "bg", "summary": "sum", "key_figures": [], '
...
class _FakeService:
    """A fake AiAgentService.extract_events() - records calls, returns
    scripted raw JSON text per call, can fail on a specific chunk index
    or trigger cancellation from inside a call."""
    ...
    def __init__(
        ...
        worker: EventExtractionWorker | None = None,
    ) -> None:
...
def test_multi_chunk_extraction_stores_real_events_and_emits_progress(tmp_path: Path, qtbot) -> None:
    ...
    service = _FakeService([_ONE_EVENT_JSON, _EMPTY_JSON, _ONE_EVENT_JSON])
    worker = EventExtractionWorker(lambda: service, repository, book_id=1, chunks=((1, 10), (11, 20), (21, 30)))
    progress_calls = []
    ...
    assert blocker.args == [2]  # 2 real events stored (chunk 2 had none)
...
def test_no_service_available_emits_unavailable_and_stores_nothing(tmp_path: Path, qtbot) -> None:
    database_path = tmp_path / "books.db"
    repository = EventCandidateRepository(database_path)
    worker = EventExtractionWorker(lambda: None, repository, book_id=1, chunks=((1, 10),))
    unavailable_calls = []
...
def test_a_failing_chunk_is_skipped_not_fatal_to_the_rest(tmp_path: Path, qtbot) -> None:
    ...
    repository = EventCandidateRepository(database_path)
    service = _FakeService([_ONE_EVENT_JSON, _ONE_EVENT_JSON, _ONE_EVENT_JSON], fail_at=1)
    worker = EventExtractionWorker(lambda: service, repository, book_id=1, chunks=((1, 10), (11, 20), (21, 30)))

    with qtbot.waitSignal(worker.extraction_finished, timeout=5000) as blocker:
        worker.start()

    assert blocker.args == [2]  # chunk 1 failed and was skipped; chunks 0 and 2 stored one event each
...
def test_cancellation_requested_mid_run_stops_further_chunks(tmp_path: Path, qtbot) -> None:
    ...
    service = _FakeService(
        [_ONE_EVENT_JSON, _ONE_EVENT_JSON, _ONE_EVENT_JSON], cancel_worker_at=0, worker=worker
    )
    ...
    # Cancellation is requested during chunk 0's own call, so chunk 0
    # still finishes and is stored; chunk 1 never starts.
