...
# Clear existing Khutbat Qasmi EventCandidates
...
print("=" * 85)
print(" DUAL-PASS EXTRACTION (INLINE + HEADING-BASED) FOR KHUTBAT QASMI")
print("=" * 85)
                ...
                if any(re.search(kw, line_clean) for kw in STORY_HEADING_KEYWORDS):
                    ...
                    if is_valid_waqia(line_clean, following_clean):
                        ...
                        ev = ExtractedEvent(
                            ...
                            key_figures=["حضرت مولانا ضیاء القاسمؒ"], quoted_excerpt=following_clean[:850], citation=cit
                        )
                        nid = event_repo.add_candidate(bid, pno, pno, ev)
                        event_repo.confirm(nid)
                        terms = [
...
print("=" * 85)
print(f" 🏆 TOTAL DUAL-PASS PURE WAQIAT INGESTED FOR KHUTBAT QASMI: {len(all_stories_collected)} WAQIAT!")
print("=" * 85)
