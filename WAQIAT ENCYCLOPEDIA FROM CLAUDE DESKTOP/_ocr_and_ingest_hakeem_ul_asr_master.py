...
OCR_BASE_DIR = r"F:\کتب\ocr text books\خطبات حکیم العصر"
...
# 12 Volumes Mapping: vol_num -> (BookID, StartPDFPage, EndPDFPage, VolumeTitle)
VOLUMES_MAP = {
    1: (3601, 1, 380, "خطبات حکیم العصر جلد 1"),
    2: (3602, 381, 759, "خطبات حکیم العصر جلد 2"),
    3: (3603, 760, 1144, "خطبات حکیم العصر جلد 3"),
    4: (3604, 1145, 1444, "خطبات حکیم العصر جلد 4"),
    5: (3605, 1445, 1793, "خطبات حکیم العصر جلد 5"),
    6: (3606, 1794, 2167, "خطبات حکیم العصر جلد 6"),
    7: (3607, 2168, 2521, "خطبات حکیم العصر جلد 7"),
    8: (3608, 2522, 2807, "خطبات حکیم العصر جلد 8"),
    9: (3609, 2808, 3153, "خطبات حکیم العصر جلد 9"),
    10: (3610, 3154, 3466, "خطبات حکیم العصر جلد 10"),
    11: (3611, 3467, 3823, "خطبات حکیم العصر جلد 11"),
    12: (3612, 3824, 4227, "خطبات حکیم العصر جلد 12")
}
...
def clean_xml_text(s):
    ...
    s = re.sub(r'خطبات\s+حکیم\s+العصر', '', s)
...
def get_volume_for_pdf_page(pdf_page):
    ...
    return 1, 3601, pdf_page, "خطبات حکیم العصر جلد 1"
    ...
    for pno in sorted(pages.keys()):
        ...
        for match in combined_pattern.finditer(clean_text):
            ...
            ev = ExtractedEvent(
                ...
                key_figures=["حضرت مولانا عبد المجید لدھیانوی مدظلہ"], quoted_excerpt=story_span, citation=cit
            )
            nid = event_repo.add_candidate(bid, pno, pno, ev)
            event_repo.confirm(nid)
            terms = [
                taxo_repo.get_or_create_term("subject", "خطباتِ حکیم العصر", "ur"),
                taxo_repo.get_or_create_term("personality", "حضرت مولانا عبد المجید لدھیانوی مدظلہ", "ur")
            ]
            ...
            if 4 < len(line_clean) < 50 and not line_clean.endswith(('۔', '!', '؟', '،')):
                if any(re.search(kw, line_clean) for kw in STORY_HEADING_KEYWORDS):
                    ...
                    if is_valid_waqia(line_clean, following_clean):
                        # Dedup
                        if any(s['page'] == pno and (s['title'] == line_clean or s['matn'][:40] in following_clean) for s in vol_stories):
                            continue
                        ...
                        ev = ExtractedEvent(
                            ...
                            key_figures=["حضرت مولانا عبد المجید لدھیانوی مدظلہ"], quoted_excerpt=following_clean[:850], citation=cit
                        )
                        nid = event_repo.add_candidate(bid, pno, pno, ev)
                        event_repo.confirm(nid)
                        terms = [
...
def main():
    ...
    parser.add_argument("--end", type=int, default=380, help="End PDF page")
    parser.add_argument("--workers", type=int, default=10, help="Concurrent workers")
    args = parser.parse_args()

    print("=" * 85)
    print(f" GOOGLE VISION OCR & INGESTION PIPELINE: KHUTBAT HAKEEM-UL-ASR (12 VOLS)")
