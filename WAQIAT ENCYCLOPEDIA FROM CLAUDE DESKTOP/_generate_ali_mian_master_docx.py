...
print(f"Total Confirmed Waqiat for Khutbat Ali Mian (7 Volumes): {len(rows)}")
...
r_title = p_title.add_run("واقعات انسائیکلوپیڈیا — خطباتِ علی میاں")
...
r_sub = p_sub.add_run(f"افادات: مفکر اسلام حضرت مولانا سید ابوالحسن علی ندویؒ (مکمل ۷ جلدیں — {urdu_num(len(rows))} مستند و خالص واقعات)")
...
out_word_path = os.path.join(OUTPUT_DIR, "واقعات انسائیکلوپیڈیا - خطبات علی میاں (مکمل ۷ جلدیں).docx")
