...
# Get all Waqiat for Khutbat-e-Faqeer (all 43 volumes)
cur.execute("""
    ...
    WHERE ec.Status = 'confirmed' AND b.Title LIKE '%خطبات فقیر%' AND b.BookID >= 3358 AND b.BookID <= 4808
    ORDER BY b.BookID, ec.ChunkStartPage
""")
rows = cur.fetchall()
conn.close()

print(f"Total Waqiat found for Khutbat-e-Faqeer: {len(rows)}")
...
r_title = p_title.add_run("واقعات انسائیکلوپیڈیا — خطباتِ فقیر")
...
r_sub = p_sub.add_run(f"پیر طریقت حضرت مولانا پیر ذوالفقار احمد نقشبندی دامت برکاتہم (مکمل ۴۳ جلدیں — {urdu_num(len(rows))} مستند اصلاحی و سوانحی واقعات)")
...
for i, (cid, bid, btitle, pno, data_json) in enumerate(rows, 1):
    ...
    w_cit = f"{btitle}، صفحہ {urdu_num(pno)}"
...
out_file = os.path.join(OUTPUT_DIR, "واقعات انسائیکلوپیڈیا - خطبات فقیر (مکمل ۴۳ جلدیں).docx")
doc.save(out_file)
print(f"✅ Generated Complete Word Document: {out_file} ({len(rows)} Waqiat)")
