...
print(f"Total Confirmed Waqiat for Khutbat Hakeem-ul-Asr (12 Volumes): {len(rows)}")
...
r_title = p_title.add_run("واقعات انسائیکلوپیڈیا — خطباتِ حکیم العصر")
...
r_sub = p_sub.add_run(f"افادات: حکیم العصر حضرت مولانا عبد المجید لدھیانوی مدظلہ (مکمل ۱۲ جلدیں — {urdu_num(len(rows))} مستند و خالص واقعات)")
...
out_word_path = os.path.join(OUTPUT_DIR, "واقعات انسائیکلوپیڈیا - خطبات حکیم العصر (مکمل ۱۲ جلدیں).docx")
