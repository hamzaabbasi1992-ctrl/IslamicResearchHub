...
- **Follow-up, same session**: Identified `D:\کتب فہرست` as the real source folder for `Jumma Bayanat` (its filenames matched existing DB titles exactly, including a `JUMMA BAYANAT` subfolder) and backfilled 2,019 more Source paths there (271 existing rows still have no matching file - left `NULL`, not guessed). Used the existing `pdf_metadata_import_cli.py` to catalog every real file on disk that had no Book row yet as a new metadata-only entry: **2,567** new Maktaba Jibreel (PDF Archive), **145** new Maktaba Al-Maknoon (PDF Archive), **365** new Jumma Bayanat - 0 failures across all three runs. Every book in these three libraries now has either a real Source path or is confirmed to have none.
- **Maktaba Islam (PDF Archive), 81 books**: Two automated fuzzy-match passes against real filenames both produced real false positives (numeric-volume-suffix similarity dominating over actual title-stem mismatch, e.g. multiple distinct `KHUTBAAT_E_FAQEER_VOL_NN` titles collapsing onto the same wrong file) and were reverted rather than trusted. Cross-checking against the rest of the database instead confirmed this library's titles are an older, differently-abbreviated catalog of content already imported elsewhere (e.g. `Islahi Taqreeren Vol 2-9` already exist with real page content under `Maktaba Al-Maknoon`). 13 of the 81 were manually verified and linked to an already-confirmed duplicate's real Source path (100% resolve to real files); the remaining 68 are left `NULL` rather than guessed.
