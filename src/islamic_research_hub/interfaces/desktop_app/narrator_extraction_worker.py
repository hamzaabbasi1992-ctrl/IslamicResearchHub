"""Background worker: extract real narrator mentions from one book, chunk
by chunk, off the GUI thread.

A real tool-calling loop per chunk (`AiAgentService.extract_narrators()`)
makes real network round trips - this must never block the GUI thread,
same reasoning as `EventExtractionWorker`/`TtsWorker`. Mirrors
`EventExtractionWorker`'s per-chunk cancellation and progress-signal shape.
"""
...
class NarratorExtractionWorker(QThread):
    """Extract real narrator mentions from one book's real chunks, off the GUI thread."""

    chunk_processed = Signal(int, int)  # done, total
    extraction_finished = Signal(int)  # total narrators stored
    extraction_unavailable = Signal(str)  # reason - service is None, not configured

    def __init__(
        self,
        get_service: Callable[[], AiAgentService | None],
        repository: NarratorCandidateRepository,
        book_id: int,
    ...
    def run(self) -> None:
        ...
        for index, (start_page, end_page) in enumerate(self._chunks):
            if self.isInterruptionRequested():
                break
            try:
                result = service.extract_narrators(self._book_id, start_page, end_page)
                narrators = parse_extracted_narrators(result.answer)
            except Exception:
                LOGGER.warning(
                    "Narrator extraction failed for book_id=%d pages %d-%d; skipping this chunk.",
                    self._book_id,
                    start_page,
                    end_page,
                )
            ...
            for narrator in narrators:
                self._repository.add_candidate(self._book_id, start_page, end_page, narrator)
                stored += 1
            self.chunk_processed.emit(index + 1, total)
