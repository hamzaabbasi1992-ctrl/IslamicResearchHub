"""Background worker: generate real study flashcards from one book,
chunk by chunk, off the GUI thread.

A real tool-calling loop per chunk (`AiAgentService.generate_flashcards()`)
makes real network round trips - this must never block the GUI thread.
Mirrors `EventExtractionWorker`'s exact shape (per-chunk cancellation,
progress signal, partial-failure-is-not-fatal discipline).
"""
...
class FlashcardExtractionWorker(QThread):
    """Generate real flashcards from one book's real chunks, off the GUI thread."""

    chunk_processed = Signal(int, int)  # done, total
    extraction_finished = Signal(int)  # total flashcards stored
    extraction_unavailable = Signal(str)  # reason - service is None, not configured

    def __init__(
        self,
        get_service: Callable[[], AiAgentService | None],
        repository: FlashcardCandidateRepository,
        book_id: int,
    ...
    def run(self) -> None:
        ...
        for index, (start_page, end_page) in enumerate(self._chunks):
            if self.isInterruptionRequested():
                break
            try:
                result = service.generate_flashcards(self._book_id, start_page, end_page)
                flashcards = parse_extracted_flashcards(result.answer)
            except Exception:
                LOGGER.warning(
                    "Flashcard generation failed for book_id=%d pages %d-%d; skipping this chunk.",
                    self._book_id,
                    start_page,
                    end_page,
                )
            ...
            for flashcard in flashcards:
                self._repository.add_candidate(self._book_id, start_page, end_page, flashcard)
                stored += 1
            self.chunk_processed.emit(index + 1, total)
